...
xyz_pos_t Probe::offset; // Initialized by settings.load()
  ...
  xy_pos_t Probe::dynamic_three_point_points[3]{
    xy_pos_t{PROBE_PT_1_X, PROBE_PT_1_Y}, 
    xy_pos_t{PROBE_PT_2_X, PROBE_PT_2_Y}, 
    xy_pos_t{PROBE_PT_3_X, PROBE_PT_3_Y}};
#endif
...
float Probe::run_z_probe(const bool sanity_check/*=true*/) {
  ...
  // Double-probing does a fast probe followed by a slow probe
    ...
    const float z = Z_CLEARANCE_DEPLOY_PROBE - offset.z;
    if (current_position.z > z) {
      // Probe down fast. If the probe triggered, raise for multi probe clearance
