...
#    if ENABLED(SENSORLESS_HOMING)
    sensorless_t stealth_states{NUM_AXIS_LIST(TERN0(X_SENSORLESS, tmc_enable_stallguard(stepperX)),
...
#    endif
    ...
    do_blocking_move_to_xy(1.5 * max_length(X_AXIS) * x_axis_home_dir,
                           1.5 * max_length(Y_AXIS) * Y_HOME_DIR,
                           fr_mm_s);
...
motion_state_t begin_slow_homing()
{
    ...
    motion_state.acceleration
        .set(planner.settings.max_acceleration_mm_per_s2[X_AXIS],
             planner.settings
                 .max_acceleration_mm_per_s2[Y_AXIS] OPTARG(DELTA,
                                                            planner.settings
                                                                .max_acceleration_mm_per_s2[Z_AXIS]));
...
// Disable the leveling matrix before homing
    ...
    const bool leveling_restore_state = parser.boolval('L',
                                                       TERN1(RESTORE_LEVELING_AFTER_G28,
...
#    if HAS_CURRENT_HOME(X) || HAS_CURRENT_HOME(X2) || HAS_CURRENT_HOME(Y) || HAS_CURRENT_HOME(Y2) \
        || (ENABLED(DELTA) && HAS_CURRENT_HOME(Z)) || HAS_CURRENT_HOME(I) || HAS_CURRENT_HOME(J) \
        || HAS_CURRENT_HOME(K) || HAS_CURRENT_HOME(U) || HAS_CURRENT_HOME(V) || HAS_CURRENT_HOME(W)
...
#    if HAS_CURRENT_HOME(X)
    const int16_t tmc_save_current_X = stepperX.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_X), tmc_save_current_X, stepperX.homing_current);
#    endif
#    if HAS_CURRENT_HOME(X2)
    const int16_t tmc_save_current_X2 = stepperX2.apply_homing_current();
    stepperX2.rms_current(X2_CURRENT_HOME);
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_X2), tmc_save_current_X2, stepperX2.homing_current);
#    endif
#    if HAS_CURRENT_HOME(Y)
    const int16_t tmc_save_current_Y = stepperY.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_Y), tmc_save_current_Y, stepperY.homing_current);
#    endif
#    if HAS_CURRENT_HOME(Y2)
    const int16_t tmc_save_current_Y2 = stepperY2.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_Y2), tmc_save_current_Y2, stepperY2.homing_current);
#    endif
#    if HAS_CURRENT_HOME(Z)
    const int16_t tmc_save_current_Z = stepperZ.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_Z), tmc_save_current_Z, stepperZ.homing_current);
#    endif
#    if HAS_CURRENT_HOME(I)
    const int16_t tmc_save_current_I = stepperI.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_I), tmc_save_current_I, stepperI.homing_current);
#    endif
#    if HAS_CURRENT_HOME(J)
    const int16_t tmc_save_current_J = stepperJ.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_J), tmc_save_current_J, stepperJ.homing_current);
#    endif
#    if HAS_CURRENT_HOME(K)
    const int16_t tmc_save_current_K = stepperK.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_K), tmc_save_current_K, stepperK.homing_current);
#    endif
#    if HAS_CURRENT_HOME(U)
    const int16_t tmc_save_current_U = stepperU.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_U), tmc_save_current_U, stepperU.homing_current);
#    endif
#    if HAS_CURRENT_HOME(V)
    const int16_t tmc_save_current_V = stepperV.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_V), tmc_save_current_V, stepperV.homing_current);
#    endif
#    if HAS_CURRENT_HOME(W)
    const int16_t tmc_save_current_W = stepperW.apply_homing_current();
    if (DEBUGGING(LEVELING))
        debug_current(F(STR_W), tmc_save_current_W, stepperW.homing_current);
#    endif
...
#    if ENABLED(PARKING_EXTRUDER)
    const bool pe_final_change_must_unpark
        = parking_extruder_unpark_after_homing(old_tool_index, X_HOME_DIR + 1 == old_tool_index * 2);
#    endif
    tool_change(0, true);
#endif
...
#    define _UNSAFE(A) (homeZ && TERN0(Z_SAFE_HOMING, axes_should_home(_BV(A##_AXIS))))
    ...
    const bool homeZ = TERN0(HAS_Z_AXIS, parser.seen_test('Z')),
               ...
               home_all = NUM_AXIS_GANG( // Home-all if all or none are flagged
                   ...
                   &&homeV == homeX,
                   &&homeW == homeX),
               NUM_AXIS_LIST(doX = home_all || homeX,
                             doY = home_all || homeY,
                             doZ = home_all || homeZ,
                             doI = home_all || homeI,
                             doJ = home_all || homeJ,
...
#    endif
    ...
    if (seenR && z_homing_height == 0) {
        if (DEBUGGING(LEVELING))
            DEBUG_ECHOLNPGM("R0 = No Z raise");
    } else if (z_homing_height && may_skate) {
        // Raise Z before homing any other axes and z is not already high enough (never lower z)
        if (DEBUGGING(LEVELING))
            DEBUG_ECHOLNPGM("Raise Z before homing:");
...
#        endif
    ...
    SECONDARY_AXIS_CODE(if (doI) homeaxis(I_AXIS),
