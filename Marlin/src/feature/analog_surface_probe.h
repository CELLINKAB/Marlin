...
struct AnalogProbe
{
    ...
    uint32_t get_distance_raw() const { return analogRead(PIN); }

    float get_distance() const { return static_cast<const Impl*>(this)->get_distance_impl(); }
    ...
    bool stow() { return static_cast<Impl*>(this)->stow_impl(); }

    bool deploy() { return static_cast<Impl*>(this)->deploy_impl(); }

    float probe_val() const { return static_cast<const Impl*>(this)->probe_val_impl(); }

#if ENABLED(GLOBAL_INTERVAL_REPORTER)
    AnalogProbe(callback_function_t reporter_)
        : reporter(reporter_)
    {}
    /**
