...
enum class TMP117_PMODE {
    THERMAL = 0,
    ALERT,
    DATA
}; //!<  Pin mode
enum class TMP117_CMODE {
    CONTINUOUS = 0,
    SHUTDOWN = 1,
    ONESHOT = 3
}; //!<  Conversion mode
enum class TMP117_CONVT {
    C15mS5 = 0,
    C125mS,
    C250mS,
    C500mS,
...
enum class TMP117_AVE {
    NOAVE = 0,
    AVE8,
    AVE32,
    AVE64
}; //!<  Averaging mode
enum class TMP117_ALERT {
    NOALERT = 0,
    HIGHALERT,
    LOWALERT
}; //!<  Alert type
enum class TMPAddr : uint8_t {
    GND = 0x48,
    SCL = 0x49,
    SDA = 0x4A,
    VCC = 0x4B,
...
template <typename Bus = TwoWire>
class TMP117
{
public:
    TMP117(TMPAddr addr, Bus &bus = Wire);
    TMP117(uint8_t addr, Bus &bus = Wire);
    void init(void (*_newDataCallback)(void));
    void update(void);
    void softReset(void);

    void setAlertMode(TMP117_PMODE mode);
    void setalertCallback(void (*alert_callback)(void), uint8_t pin);
