 ...
 * initData is the first data for the CRC calculation (constant)
 * c_ptr is a pointer to a uint8_t array of data
 * len is the number of bytes
 */
constexpr uint16_t crcCalculate16(uint16_t initData, const void *c_ptr, size_t len)
{
	...
	while (len--)
	{
		const uint16_t magic = crcTable16[((crc >> 8) ^ *c++)];
		crc = (crc << 8) ^ magic ;
	}
