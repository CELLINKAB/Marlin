...
bool regulator::setOutputPressure(uint16_t pressureValue, uint8_t regulator)
{
	...
	if(regulatorSetOffset[regulator-1]<0 && pressureValue<abs(regulatorSetOffset[regulator-1]))
	{
		setPWM(0,regulator);
		return true;
	}
...
uint16_t regulator::getPressureFeedback(uint8_t regulator)
{
	...
	uint32_t regPress=(((uint32_t)feedbackVoltage-PRESSURE_FEEDBACK_MIN_VOLT)*fullScalePressure)/(PRESSURE_FEEDBACK_MAX_VOLT-PRESSURE_FEEDBACK_MIN_VOLT);
...
void airSystemLoadConfig(bool resetParams /*= false*/)
{
	...
	{
		//Read all data
		const configErr_t err = configReadDataSet(CONFIG_GET_ADDR(CONFIG_SYS_PRESSURE), AIR_SYSTEM_CALIB_VERSION, (uint8_t*)(&tmp), sizeof(pressureStoreParams_t));
...
void airSystemStoreConfig(bool reset /*= false*/)
{
	...
	//Write to memory
	configWriteDataSet(CONFIG_GET_ADDR(CONFIG_SYS_PRESSURE), (uint8_t*)(&tmp), sizeof(pressureStoreParams_t));
}
