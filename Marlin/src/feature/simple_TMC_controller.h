...
{
    constexpr SimpleTMCConfig(uint8_t address, uint8_t sensitivity) : hw_address(address), stall_sensitivity(sensitivity), rms_current(600) {}
    constexpr SimpleTMCConfig(uint8_t address, uint8_t sensitivity, uint32_t current) : hw_address(address), stall_sensitivity(sensitivity), rms_current(current) {}
    uint8_t hw_address;
...
private:
     ...
     */
    SimpleTMC(TMCMarlin<TMC2209Stepper, 'N', '0', AxisEnum::NO_AXIS_ENUM> *driver_) : driver(driver_)
    {
        OUT_WRITE(EN, HIGH);
...
struct SimpleTMC<EN, STOP, INDEX, false>
{
    ...
    {
        static TMCMarlin<TMC2209Stepper, 'N', '0', AxisEnum::NO_AXIS_ENUM> driver(SIMPLE_TMC_SERIAL, 0.11f, config.hw_address);
