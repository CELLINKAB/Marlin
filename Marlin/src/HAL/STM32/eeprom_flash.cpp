...
#                define FLASH_ADDRESS_START \
                    (FLASH_END - ((FLASH_SECTOR_TOTAL - (FLASH_SECTOR)) * (FLASH_UNIT_SIZE)) + 1)
#            endif
...
static_assert(0 == MARLIN_EEPROM_SIZE % 4,
              "MARLIN_EEPROM_SIZE must be a multiple of 4"); // Ensure copying as uint32_t is safe
static_assert(0 == FLASH_UNIT_SIZE % MARLIN_EEPROM_SIZE,
              "MARLIN_EEPROM_SIZE must divide evenly into your FLASH_UNIT_SIZE");
static_assert(FLASH_UNIT_SIZE >= MARLIN_EEPROM_SIZE,
              "FLASH_UNIT_SIZE must be greater than or equal to your MARLIN_EEPROM_SIZE");
...
#        if ENABLED(FLASH_EEPROM_LEVELING)
    ...
    if (current_slot == -1 || eeprom_data_written) {
        ...
        if (eeprom_data_written)
            DEBUG_ECHOLNPGM("Dangling EEPROM write_data");
        ...
        if (current_slot == -1) {
            ...
            for (int i = 0; i < MARLIN_EEPROM_SIZE; i++)
                ram_eeprom[i] = EMPTY_UINT8;
            current_slot = EEPROM_SLOTS;
        } else {
            // load current settings
            uint8_t* eeprom_data = (uint8_t*) SLOT_ADDRESS(current_slot);
            for (int i = 0; i < MARLIN_EEPROM_SIZE; i++)
                ram_eeprom[i] = eeprom_data[i];
...
#        if ENABLED(FLASH_EEPROM_LEVELING)
        ...
        uint32_t offset = 0, address = SLOT_ADDRESS(current_slot),
                 address_end = address + MARLIN_EEPROM_SIZE, data = 0;
...
#            endif
        ...
        eeprom_buffer_flush();
        hal.isr_on();
        if (auto err = HAL_FLASH_GetError(); err != 0)
            DEBUG_ECHOLNPGM("Error during write: ", err);
        TERN_(HAS_PAUSE_SERVO_OUTPUT, RESUME_SERVO_OUTPUT());
...
bool PersistentStore::read_data(
    int& pos, uint8_t* value, size_t size, uint16_t* crc, const bool writing /*=true*/)
{
    do {
